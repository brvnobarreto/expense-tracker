...
export default function Home() {
  ...
  const handleEditExpense = (expense: Expense) => {
    const formattedDate = expense.date ? new Date(expense.date).toISOString().split("T")[0] : ""; // Formata a data
    setNewExpense({ ...expense, date: formattedDate }); // Atualiza o estado com a despesa
    setIsEditing(true); // Modo de edição
    setModalOpen(true); // Abre o modal
  };

  const handleAddExpense = () => {
    setNewExpense({ id: 0, name: "", amount: 0, date: "" }); // Reseta os valores para adicionar uma nova despesa
    setIsEditing(false); // Modo de criação (adicionar)
    setModalOpen(true); // Abre o modal
  };
  ...
  return (
    <div className="p-6 max-w-xl mx-auto">
      ...
      <div className="flex justify-between mt-4 p-4 bg-gray-100 rounded-lg">
        <span>
          Saldo:{" "}
          <span className={balance < 0 ? "text-red-500" : ""}>R$ {balance.toFixed(2)}</span>
        </span>
        <span>
          Total de Gastos:{" "}
